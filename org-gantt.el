...
(ert-deftest ert-org-gantt-timestamp-to-time ()
  (let (count year month day hour minute tstring tvalue tstamp tvalue)
    (setq count 100)
    (while (/= count 0)
      (setq count (1- count))
      (setq year (+ 1 (random 9998))
            month (+ 1 (random 12)))
      (if (member month (list 1 3 5 7 8 10 12))
          (setq day (+ 1 (random 31)))
        (if (= month 2)
            (setq day (+ 1 (random 28)))
          (setq day (+ 1 (random 30)))))
      (setq hour (+ 1 (random 23))
            minute (+ 1 (random 59)))
      (setq tstring (format "<%04d-%02d-%02d %02d:%02d>" year month day hour minute)
            tstamp (list 'timestamp (nconc (list :raw-value tstring)))
            tvalue (org-gantt-timestamp-to-time tstamp))
      ;(message tstring)
      (should (= (string-to-number (format-time-string "%Y" tvalue)) year))
      (should (= (string-to-number (format-time-string "%m" tvalue)) month))
      (should (= (string-to-number (format-time-string "%d" tvalue)) day)))))

(ert-deftest ert-org-gantt-duration-to-day ()
  (let (count n nstring duration unit)
    (setq count 100)
    (while (/= count 0)
      (setq count (1- count))
      (dolist (unit (list "d" "w" "m"))
        (setq n (random 365)
              nstring (format "%d%s" n unit))
        (setq duration (org-gantt-duration-to-day nstring))
        (cond ((string= unit "m")
               (setq n (* n 30)))
              ((string= unit "w")
               (setq n (* n 7)))
